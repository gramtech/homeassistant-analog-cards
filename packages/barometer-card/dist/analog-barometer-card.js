...
`;const zt="analog-barometer-card",Mt="analog-barometer-card-editor",Ut={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Ot={startAngle:-135,sweepAngle:270},Dt=[{key:"stormy",label:"Stormy",from:950,to:980,color:"#8b1a1a"},{key:"rain",label:"Rain",from:980,to:1e3,color:"#b5651d"},{key:"change",label:"Change",from:1e3,to:1015,color:"#c9a227"},{key:"fair",label:"Fair",from:1015,to:1030,color:"#4a7a3c"},{key:"v-dry",label:"Very Dry",from:1030,to:1050,color:"#2f5f4f"}],Ht={hPa:{major:10,minor:2},inHg:{major:.5,minor:.1}},jt=33.8639,Tt=new Set(["inhg","inHg","in"]),Nt=new Set(["hpa","mbar","mb"]);function Rt(t){return t/jt}function Lt(t,e){return"inHg"===e?Rt(t):t}function Ft(t,e){return"inHg"===e?t*jt:t}function It(t,e,i){return e===i?t:Lt(Ft(t,e),i)}function Vt(t,e){const i=t.states[e]?.attributes?.unit_of_measurement;if(!i)return"hPa";const s=i.toLowerCase();return Tt.has(s)||s.includes("inhg")?"inHg":(Nt.has(s),"hPa")}const Bt=100;function qt(t){const{min:e,max:i,unit:s,value:r,pastValue:n,needleColor:a}=t;return q`
  ...
  `}</defs>
      <circle cx=${Bt} cy=${Bt} r="97" class="bezel" />
      <circle cx=${Bt} cy=${Bt} r="90" class="face" />
      ${function(t,e,i){return Dt.map(s=>{const r=Lt(s.from,i),n=Lt(s.to,i);if(n<=t||r>=e)return q``;const a=kt(r,t,e,Ot),o=kt(n,t,e,Ot);if(o<=a)return q``;const l=_t(Bt,Bt,86,a,o);return q`<path
      d=${l}
      stroke=${s.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i,s)}
      ${function(t,e,i){const{major:s,minor:r}=Ht[i];return vt(t,e,s,r,Ot).map(t=>{const e=t.major?8:4,i=yt(Bt,Bt,72-e,t.angle),s=yt(Bt,Bt,72,t.angle),r=q`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const n=yt(Bt,Bt,50,t.angle),a=q`<text x=${n.x} y=${n.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return q`${r}${a}`})}(e,i,s)}
      ${function(t,e,i){return Dt.map(s=>{const r=Math.max(Lt(s.from,i),t),n=Math.min(Lt(s.to,i),e);if(n<=r)return q``;const a=kt(r,t,e,Ot),o=kt(n,t,e,Ot),l=`zone-label-path-${s.key}`,c=_t(Bt,Bt,77,a,o);return q`
    ...
    `})}(e,i,s)}
      ${function(t,e,i){if(null===t)return q``;const s=kt(t,e,i,Ot),r=yt(Bt,Bt,88,s),n=yt(Bt,Bt,95,s);return q`<line
    x1=${r.x} y1=${r.y} x2=${n.x} y2=${n.y}
    class="trend-marker"
  />`}(n,e,i)}
      ${function(t,e,i,s){if(null===t)return q``;const r=kt(t,e,i,Ot);return q`<g class="needle" transform="rotate(${r} ${Bt} ${Bt})">
    <polygon
      points="${Bt},${38} ${97},${114} ${103},${114}"
      fill=${s}
    />
  </g>`}(r,e,i,a)}
      <circle cx=${Bt} cy=${Bt} r="6" class="hub" />
    </svg>
  `}const Wt=[(Kt="sensor",{name:"entity",selector:{entity:{domain:Kt}}}),wt("name"),Et("unit",[{value:"auto",label:"Auto-detect"},{value:"hPa",label:"hPa"},{value:"inHg",label:"inHg"}]),xt("trend_hours",{min:1,max:12,step:1}),xt("trend_threshold",{min:0,step:.1}),Et("face_style",At),wt("needle_color")];var Kt;const Gt=(Jt={entity:"Pressure sensor entity",name:"Name (optional)",unit:"Display unit",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>Jt[t.name]??t.name);var Jt;let Zt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?B`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Wt}
      ...
      ></ha-form>
    `:B``}_valueChanged(t){!function(t,e,i={},s={}){const r=new CustomEvent(e,{bubbles:s.bubbles??!0,cancelable:s.cancelable??!1,composed:s.composed??!0,detail:i});t.dispatchEvent(r)}(this,"config-changed",{config:t.detail.value})}};t([gt({attribute:!1})],Zt.prototype,"hass",void 0),t([pt()],Zt.prototype,"_config",void 0),Zt=t([ut(Mt)],Zt);const Yt={rising:"↑",falling:"↓",steady:"→",unknown:""},Qt={rising:"Rising",falling:"Falling",steady:"Steady",unknown:""};let Xt=class extends ct{constructor(){super(...arguments),this._trendDirection="unknown",this._pastValueDisplay=null,this._tracker=new Ct("analog-barometer-card-trend-",It,3e5),this._lastEntityUpdate=null}setConfig(t){if(!t.entity)throw new Error("You must specify a pressure sensor entity.");this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement(Mt)}static getStubConfig(t){const e=Object.values(t?.states??{}).find(t=>t.entity_id.startsWith("sensor.")&&"atmospheric_pressure"===t.attributes.device_class);return{type:"custom:analog-barometer-card",entity:e?.entity_id??""}}willUpdate(t){t.has("hass")&&this._maybeUpdateTrend()}_maybeUpdateTrend(){if(!this.hass||!this._config?.entity)return;const t=this.hass.states[this._config.entity];if(!t)return;const e=parseFloat(t.state);if(!Number.isFinite(e))return;const i=Vt(this.hass,this._config.entity),s=this._config.unit??i,r=It(e,i,s),n=this._config.trend_hours??3,a=this._config.trend_threshold??("inHg"===s?Rt(1.5):1.5);if(this._lastEntityUpdate===t.last_updated)return;this._lastEntityUpdate=t.last_updated;const o=this._tracker.update(this.hass,this._config.entity,r,i,s,n,a,t=>{this._trendDirection=t.direction,this._pastValueDisplay=t.pastValueDisplay});this._trendDirection=o?.direction??"unknown",this._pastValueDisplay=o?.pastValueDisplay??null}render(){if(!this._config||!this.hass)return B``;const t=this.hass.states[this._config.entity];if(!t)return B`<ha-card>
        <div class="warning">Entity not found: ${this._config.entity}</div>
      </ha-card>`;const e=Vt(this.hass,this._config.entity),i=this._config.unit??e,s=parseFloat(t.state),r=Number.isFinite(s),n=r?It(s,e,i):null,a="inHg"===i?{min:Rt(950),max:Rt(1050)}:{min:950,max:1050};const o=this._config.min??a.min,l=this._config.max??a.max,c=this._config.face_style??"classic",h=this._config.needle_color||Ut[c],u=this._config.name??t.attributes.friendly_name??this._config.entity;return B`
      <ha-card>
        <div class=${mt({"card-content":!0,[`style-${c}`]:!0})}>
          <div class="title">${u}</div>
          ${qt({min:o,max:l,unit:i,value:n,pastValue:this._pastValueDisplay,needleColor:h})}
          <div class="readout">
            <span class="value">${r?n.toFixed(1):"--"}</span>
            <span class="unit">${i}</span>
          </div>
          <div class="trend trend-${this._trendDirection}">
            <span class="arrow">${Yt[this._trendDirection]}</span>
            <span class="label">${Qt[this._trendDirection]}</span>
          </div>
        </div>
      </ha-card>
    `}};var te;Xt.styles=[Pt,a`
      :host {
        display: block;
      }
      .card-content {
    ...
    `],t([gt({attribute:!1})],Xt.prototype,"hass",void 0),t([pt()],Xt.prototype,"_config",void 0),t([pt()],Xt.prototype,"_trendDirection",void 0),t([pt()],Xt.prototype,"_pastValueDisplay",void 0),Xt=t([ut(zt)],Xt),te={type:zt,name:"Analog Barometer",preview:!1,description:"A classic analog-style barometer with pressure trend tracking."},window.customCards=window.customCards||[],window.customCards.push(te);export{Xt as AnalogBarometerCard};
