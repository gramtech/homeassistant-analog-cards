...
`;const Ut="analog-barometer-card",Ot="analog-barometer-card-editor",Dt={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Ht={startAngle:-135,sweepAngle:270},Tt=[{key:"stormy",label:"Stormy",from:950,to:980,color:"#8b1a1a"},{key:"rain",label:"Rain",from:980,to:1e3,color:"#b5651d"},{key:"change",label:"Change",from:1e3,to:1015,color:"#c9a227"},{key:"fair",label:"Fair",from:1015,to:1030,color:"#4a7a3c"},{key:"v-dry",label:"Very Dry",from:1030,to:1050,color:"#2f5f4f"}],jt={hPa:{major:10,minor:2},inHg:{major:.5,minor:.1}},Nt=33.8639,Rt=new Set(["inhg","inHg","in"]),Lt=new Set(["hpa","mbar","mb"]);function Ft(t){return t/Nt}function It(t,e){return"inHg"===e?Ft(t):t}function Vt(t,e){return"inHg"===e?t*Nt:t}function Bt(t,e,i){return e===i?t:It(Vt(t,e),i)}function qt(t,e){const i=t.states[e]?.attributes?.unit_of_measurement;if(!i)return"hPa";const s=i.toLowerCase();return Rt.has(s)||s.includes("inhg")?"inHg":(Lt.has(s),"hPa")}const Wt=100;function Kt(t){const{min:e,max:i,unit:s,value:r,pastValue:n,needleColor:a}=t;return q`
  ...
  `}</defs>
      ...
      ${function(t,e,i){return Tt.map(s=>{const r=It(s.from,i),n=It(s.to,i);if(n<=t||r>=e)return q``;const a=kt(r,t,e,Ht),o=kt(n,t,e,Ht);if(o<=a)return q``;const l=yt(Wt,Wt,86,a,o);return q`<path
      d=${l}
      stroke=${s.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i,s)}
      ${function(t,e,i){const{major:s,minor:r}=jt[i];return vt(t,e,s,r,Ht).map(t=>{const e=t.major?8:4,i=_t(Wt,Wt,72-e,t.angle),s=_t(Wt,Wt,72,t.angle),r=q`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const n=_t(Wt,Wt,50,t.angle),a=q`<text x=${n.x} y=${n.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return q`${r}${a}`})}(e,i,s)}
      ${function(t,e,i){return Tt.map(s=>{const r=Math.max(It(s.from,i),t),n=Math.min(It(s.to,i),e);if(n<=r)return q``;const a=kt(r,t,e,Ht),o=kt(n,t,e,Ht),l=`zone-label-path-${s.key}`,c=yt(Wt,Wt,77,a,o);return q`
    ...
    `})}(e,i,s)}
      ${function(t,e,i){if(null===t)return q``;const s=kt(t,e,i,Ht),r=_t(Wt,Wt,88,s),n=_t(Wt,Wt,95,s);return q`<line
    x1=${r.x} y1=${r.y} x2=${n.x} y2=${n.y}
    class="trend-marker"
  />`}(n,e,i)}
      ${function(t,e,i,s){if(null===t)return q``;const r=kt(t,e,i,Ht);return q`<g class="needle" transform="rotate(${r} ${Wt} ${Wt})">
    <polygon
  ...
  `}const Gt=[(Jt="sensor",{name:"entity",selector:{entity:{domain:Jt}}}),Et("name"),Ct("unit",[{value:"auto",label:"Auto-detect"},{value:"hPa",label:"hPa"},{value:"inHg",label:"inHg"}]),St("trend_hours",{min:1,max:12,step:1}),St("trend_threshold",{min:0,step:.1}),Ct("trend_text_size",wt),Ct("face_style",At),Et("needle_color")];var Jt;const Zt=(Yt={entity:"Pressure sensor entity",name:"Name (optional)",unit:"Display unit",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",trend_text_size:"Trend text size",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>Yt[t.name]??t.name);var Yt;let Qt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?B`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Gt}
    ...
    `],t([gt({attribute:!1})],ee.prototype,"hass",void 0),t([pt()],ee.prototype,"_config",void 0),t([pt()],ee.prototype,"_trendDirection",void 0),t([pt()],ee.prototype,"_pastValueDisplay",void 0),ee=t([ut(Ut)],ee),ie={type:Ut,name:"Analog Barometer",preview:!1,description:"A classic analog-style barometer with pressure trend tracking."},window.customCards=window.customCards||[],window.customCards.push(ie);export{ee as AnalogBarometerCard};
