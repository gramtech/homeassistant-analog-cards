 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */const $t=(t=>(...e)=>({_$litDirective$:t,values:e}))(class extends mt{constructor(t){if(super(t),t.type!==bt||"class"!==t.name||t.strings?.length>2)throw Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.")}render(t){return" "+Object.keys(t).filter(e=>t[e]).join(" ")+" "}update(t,[e]){if(void 0===this.st){this.st=new Set,void 0!==t.strings&&(this.nt=new Set(t.strings.join(" ").split(/\s/).filter(t=>""!==t)));for(const t in e)e[t]&&!this.nt?.has(t)&&this.st.add(t);return this.render(e)}const i=t.element.classList;for(const t of this.st)t in e||(i.remove(t),this.st.delete(t));for(const t in e){const s=!!e[t];s===this.st.has(t)||this.nt?.has(t)||(s?(i.add(t),this.st.add(t)):(i.remove(t),this.st.delete(t)))}return W}});function yt(t,e,i,s){const r=(s-90)*Math.PI/180;return{x:t+i*Math.cos(r),y:e+i*Math.sin(r)}}function _t(t,e,i,s,r){const a=yt(t,e,i,s),n=yt(t,e,i,r),o=r-s<=180?0:1;return`M ${a.x} ${a.y} A ${i} ${i} 0 ${o} 1 ${n.x} ${n.y}`}function kt(t,e,i,s){if(i<=e)return s.startAngle;let r;if(s.wrap){const s=i-e;r=((t-e)%s+s)%s/s}else{r=(Math.min(i,Math.max(e,t))-e)/(i-e)}return s.startAngle+r*s.sweepAngle}function vt(t,e,i,s,r){return function(t,e,i,s){if(e<=t)return[];const r=[],a=s/1e3;for(let n=t;n<=e+a;n+=s){const t=Math.round(n/s)*s,e=Math.abs(t/i-Math.round(t/i))<a;r.push({value:t,major:e})}return r}(t,e,i,s).map(i=>({...i,angle:kt(i.value,t,e,r)}))}const At=[{value:"classic",label:"Classic"},{value:"silver",label:"Silver"},{value:"dark",label:"Dark"},{value:"light",label:"Light"},{value:"led-backlit",label:"LED Backlit"},{value:"fluorescent",label:"Fluorescent"},{value:"futuristic",label:"Futuristic"}],xt=[{value:"small",label:"Small"},{value:"medium",label:"Medium"},{value:"large",label:"Large"},{value:"x-large",label:"Extra large"}],wt={small:.8,medium:.9,large:1.05,"x-large":1.2};function Et(t){return{name:t,selector:{text:{}}}}function St(t,e){return{name:t,selector:{number:{...e,mode:"box"}}}}function Ct(t,e){return{name:t,selector:{select:{mode:"dropdown",options:e}}}}function Pt(t,e,i){const s=t-e;return Math.abs(s)<i?"steady":s>0?"rising":"falling"}class zt{constructor(t,e,i=3e5){this.storageKeyPrefix=t,this.convert=e,this.throttleMs=i,this.cache=null,this.cachedEntityId=null,this.inFlight=null}storageKey(t){return`${this.storageKeyPrefix}${t}`}getCached(t){return this.cachedEntityId!==t&&(this.cachedEntityId=t,this.cache=function(t){try{const e=localStorage.getItem(t);return e?JSON.parse(e):null}catch{return null}}(this.storageKey(t))),this.cache}shouldFetch(t){const e=this.getCached(t);return!e||Date.now()-e.fetchedAt>this.throttleMs}update(t,e,i,s,r,a,n,o){const l=this.getCached(e);return this.inFlight||!this.shouldFetch(e)||(this.inFlight=async function(t,e,i){const s=`history/period/${new Date(Date.now()-36e5*i).toISOString()}?filter_entity_id=${encodeURIComponent(e)}&minimal_response&no_attributes`,r=await t.callApi("GET",s),a=r?.[0];if(!a?.length)return null;const n=a[0],o=n.state??n.s;if(void 0===o)return null;const l=parseFloat(o);return Number.isFinite(l)?l:null}(t,e,a).then(t=>{if(null===t)return{direction:"unknown",pastValueDisplay:null,fetchedAt:Date.now()};const e=this.convert(t,s,r);return{direction:Pt(i,e,n),pastValueDisplay:e,fetchedAt:Date.now()}}).catch(()=>({direction:l?.direction??"unknown",pastValueDisplay:l?.pastValueDisplay??null,fetchedAt:Date.now()})).then(t=>(this.cache=t,function(t,e){try{localStorage.setItem(t,JSON.stringify(e))}catch{}}(this.storageKey(e),t),this.inFlight=null,o?.(t),t))),l}}const Mt=n`
  .style-classic {
...
`;const Ut="analog-humidity-radial-card",Ot="analog-humidity-radial-card-editor",Dt={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Tt={startAngle:-135,sweepAngle:270},jt=[{key:"dry",label:"Dry",from:0,to:30,color:"#b5651d"},{key:"comfortable",label:"Comfortable",from:30,to:50,color:"#4a7a3c"},{key:"humid",label:"Humid",from:50,to:70,color:"#c9a227"},{key:"very-humid",label:"Very Humid",from:70,to:100,color:"#2471a3"}],Ht={major:10,minor:2},Nt=100;function Rt(t){const{min:e,max:i,value:s,pastValue:r,needleColor:a}=t;return B`
  ...
  `}</defs>
      <circle cx=${Nt} cy=${Nt} r="97" class="bezel" />
      <circle cx=${Nt} cy=${Nt} r="90" class="face" />
      ${function(t,e){return jt.map(i=>{if(i.to<=t||i.from>=e)return B``;const s=kt(i.from,t,e,Tt),r=kt(i.to,t,e,Tt);if(r<=s)return B``;const a=_t(Nt,Nt,86,s,r);return B`<path
      d=${a}
      stroke=${i.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i)}
      ${function(t,e){const{major:i,minor:s}=Ht;return vt(t,e,i,s,Tt).map(t=>{const e=t.major?8:4,i=yt(Nt,Nt,72-e,t.angle),s=yt(Nt,Nt,72,t.angle),r=B`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const a=yt(Nt,Nt,50,t.angle),n=B`<text x=${a.x} y=${a.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${r}${n}`})}(e,i)}
      ${function(t,e){return jt.map(i=>{const s=Math.max(i.from,t),r=Math.min(i.to,e);if(r<=s)return B``;const a=kt(s,t,e,Tt),n=kt(r,t,e,Tt),o=`zone-label-path-${i.key}`,l=_t(Nt,Nt,77,a,n);return B`
    ...
    `})}(e,i)}
      ${function(t,e,i){if(null===t)return B``;const s=kt(t,e,i,Tt),r=yt(Nt,Nt,88,s),a=yt(Nt,Nt,95,s);return B`<line
    x1=${r.x} y1=${r.y} x2=${a.x} y2=${a.y}
    class="trend-marker"
  />`}(r,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=kt(t,e,i,Tt);return B`<g class="needle" transform="rotate(${r} ${Nt} ${Nt})">
    <polygon
      points="${Nt},${38} ${97},${114} ${103},${114}"
      fill=${s}
    />
  </g>`}(s,e,i,a)}
      <circle cx=${Nt} cy=${Nt} r="6" class="hub" />
    </svg>
  `}function Lt(t,e,i){return t}const It=[(Vt="sensor",{name:"entity",selector:{entity:{domain:Vt}}}),Et("name"),St("trend_hours",{min:1,max:12,step:1}),St("trend_threshold",{min:0,step:.5}),Ct("trend_text_size",xt),Ct("face_style",At),Et("needle_color")];var Vt;const Ft=(qt={entity:"Humidity sensor entity",name:"Name (optional)",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",trend_text_size:"Trend text size",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>qt[t.name]??t.name);var qt;let Bt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?q`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${It}
      ...
      ></ha-form>
    `:q``}_valueChanged(t){!function(t,e,i={},s={}){const r=new CustomEvent(e,{bubbles:s.bubbles??!0,cancelable:s.cancelable??!1,composed:s.composed??!0,detail:i});t.dispatchEvent(r)}(this,"config-changed",{config:t.detail.value})}};t([gt({attribute:!1})],Bt.prototype,"hass",void 0),t([pt()],Bt.prototype,"_config",void 0),Bt=t([ut(Ot)],Bt);const Wt={rising:"↑",falling:"↓",steady:"→",unknown:""},Kt={rising:"Rising",falling:"Falling",steady:"Steady",unknown:""};let Gt=class extends ct{constructor(){super(...arguments),this._trendDirection="unknown",this._pastValueDisplay=null,this._tracker=new zt("analog-humidity-radial-card-trend-",Lt,3e5),this._lastEntityUpdate=null}setConfig(t){if(!t.entity)throw new Error("You must specify a humidity sensor entity.");this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement(Ot)}static getStubConfig(t){const e=Object.values(t?.states??{}).find(t=>t.entity_id.startsWith("sensor.")&&"humidity"===t.attributes.device_class);return{type:"custom:analog-humidity-radial-card",entity:e?.entity_id??""}}willUpdate(t){t.has("hass")&&this._maybeUpdateTrend()}_maybeUpdateTrend(){if(!this.hass||!this._config?.entity)return;const t=this.hass.states[this._config.entity];if(!t)return;const e=parseFloat(t.state);if(!Number.isFinite(e))return;const i=(this.hass,this._config.entity,"%"),s=Lt(e),r=this._config.trend_hours??3,a=this._config.trend_threshold??3;if(this._lastEntityUpdate===t.last_updated)return;this._lastEntityUpdate=t.last_updated;const n=this._tracker.update(this.hass,this._config.entity,s,i,i,r,a,t=>{this._trendDirection=t.direction,this._pastValueDisplay=t.pastValueDisplay});this._trendDirection=n?.direction??"unknown",this._pastValueDisplay=n?.pastValueDisplay??null}render(){if(!this._config||!this.hass)return q``;const t=this.hass.states[this._config.entity];if(!t)return q`<ha-card>
        <div class="warning">Entity not found: ${this._config.entity}</div>
      </ha-card>`;const e=(this.hass,this._config.entity,"%"),i=parseFloat(t.state),s=Number.isFinite(i),r=s?Lt(i):null,a={min:0,max:100},n=this._config.min??a.min,o=this._config.max??a.max,l=this._config.face_style??"classic",c=this._config.needle_color||Dt[l],h=this._config.name??t.attributes.friendly_name??this._config.entity,u=wt[this._config.trend_text_size??"large"];return q`
      <ha-card>
        <div class=${$t({"card-content":!0,[`style-${l}`]:!0})}>
          <div class="title">${h}</div>
          ${Rt({min:n,max:o,value:r,pastValue:this._pastValueDisplay,needleColor:c})}
          <div class="readout">
            <span class="value">${s?r.toFixed(0):"--"}</span>
            <span class="unit">${e}</span>
          </div>
          <div class="trend trend-${this._trendDirection}" style="font-size: ${u}rem">
            <span class="arrow">${Wt[this._trendDirection]}</span>
            <span class="label">${Kt[this._trendDirection]}</span>
          </div>
        </div>
      </ha-card>
    `}};var Jt;Gt.styles=[Mt,n`
      :host {
        display: block;
      }
      .card-content {
    ...
    `],t([gt({attribute:!1})],Gt.prototype,"hass",void 0),t([pt()],Gt.prototype,"_config",void 0),t([pt()],Gt.prototype,"_trendDirection",void 0),t([pt()],Gt.prototype,"_pastValueDisplay",void 0),Gt=t([ut(Ut)],Gt),Jt={type:Ut,name:"Analog Humidity (Radial)",preview:!1,description:"A classic analog-style radial hygrometer dial with humidity trend tracking."},window.customCards=window.customCards||[],window.customCards.push(Jt);export{Gt as AnalogHumidityRadialCard};
