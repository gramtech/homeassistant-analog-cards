...
`;const Pt="analog-humidity-radial-card",zt="analog-humidity-radial-card-editor",Mt={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Ut={startAngle:-135,sweepAngle:270},Ot=[{key:"dry",label:"Dry",from:0,to:30,color:"#b5651d"},{key:"comfortable",label:"Comfortable",from:30,to:50,color:"#4a7a3c"},{key:"humid",label:"Humid",from:50,to:70,color:"#c9a227"},{key:"very-humid",label:"Very Humid",from:70,to:100,color:"#2471a3"}],Dt={major:10,minor:2},Tt=100;function jt(t){const{min:e,max:i,value:s,pastValue:r,needleColor:n}=t;return B`
  ...
  `}</defs>
      <circle cx=${Tt} cy=${Tt} r="97" class="bezel" />
      <circle cx=${Tt} cy=${Tt} r="90" class="face" />
      ${function(t,e){return Ot.map(i=>{if(i.to<=t||i.from>=e)return B``;const s=kt(i.from,t,e,Ut),r=kt(i.to,t,e,Ut);if(r<=s)return B``;const n=_t(Tt,Tt,86,s,r);return B`<path
      d=${n}
      stroke=${i.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i)}
      ${function(t,e){const{major:i,minor:s}=Dt;return vt(t,e,i,s,Ut).map(t=>{const e=t.major?8:4,i=yt(Tt,Tt,72-e,t.angle),s=yt(Tt,Tt,72,t.angle),r=B`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const n=yt(Tt,Tt,50,t.angle),a=B`<text x=${n.x} y=${n.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${r}${a}`})}(e,i)}
      ${function(t,e){return Ot.map(i=>{const s=Math.max(i.from,t),r=Math.min(i.to,e);if(r<=s)return B``;const n=kt(s,t,e,Ut),a=kt(r,t,e,Ut),o=`zone-label-path-${i.key}`,l=_t(Tt,Tt,77,n,a);return B`
    ...
    `})}(e,i)}
      ${function(t,e,i){if(null===t)return B``;const s=kt(t,e,i,Ut),r=yt(Tt,Tt,88,s),n=yt(Tt,Tt,95,s);return B`<line
    x1=${r.x} y1=${r.y} x2=${n.x} y2=${n.y}
    class="trend-marker"
  />`}(r,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=kt(t,e,i,Ut);return B`<g class="needle" transform="rotate(${r} ${Tt} ${Tt})">
    <polygon
      points="${Tt},${38} ${97},${114} ${103},${114}"
      fill=${s}
    />
  </g>`}(s,e,i,n)}
      <circle cx=${Tt} cy=${Tt} r="6" class="hub" />
    </svg>
  `}function Ht(t,e,i){return t}const Nt=[(Rt="sensor",{name:"entity",selector:{entity:{domain:Rt}}}),wt("name"),xt("trend_hours",{min:1,max:12,step:1}),xt("trend_threshold",{min:0,step:.5}),{name:"face_style",selector:{select:{mode:"dropdown",options:At}}},wt("needle_color")];var Rt;const It=(Lt={entity:"Humidity sensor entity",name:"Name (optional)",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>Lt[t.name]??t.name);var Lt;let Vt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?q`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Nt}
      ...
      ></ha-form>
    `:q``}_valueChanged(t){!function(t,e,i={},s={}){const r=new CustomEvent(e,{bubbles:s.bubbles??!0,cancelable:s.cancelable??!1,composed:s.composed??!0,detail:i});t.dispatchEvent(r)}(this,"config-changed",{config:t.detail.value})}};t([gt({attribute:!1})],Vt.prototype,"hass",void 0),t([pt()],Vt.prototype,"_config",void 0),Vt=t([ut(zt)],Vt);const Ft={rising:"↑",falling:"↓",steady:"→",unknown:""},qt={rising:"Rising",falling:"Falling",steady:"Steady",unknown:""};let Bt=class extends ct{constructor(){super(...arguments),this._trendDirection="unknown",this._pastValueDisplay=null,this._tracker=new St("analog-humidity-radial-card-trend-",Ht,3e5),this._lastEntityUpdate=null}setConfig(t){if(!t.entity)throw new Error("You must specify a humidity sensor entity.");this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement(zt)}static getStubConfig(t){const e=Object.values(t?.states??{}).find(t=>t.entity_id.startsWith("sensor.")&&"humidity"===t.attributes.device_class);return{type:"custom:analog-humidity-radial-card",entity:e?.entity_id??""}}willUpdate(t){t.has("hass")&&this._maybeUpdateTrend()}_maybeUpdateTrend(){if(!this.hass||!this._config?.entity)return;const t=this.hass.states[this._config.entity];if(!t)return;const e=parseFloat(t.state);if(!Number.isFinite(e))return;const i=(this.hass,this._config.entity,"%"),s=Ht(e),r=this._config.trend_hours??3,n=this._config.trend_threshold??3;if(this._lastEntityUpdate===t.last_updated)return;this._lastEntityUpdate=t.last_updated;const a=this._tracker.update(this.hass,this._config.entity,s,i,i,r,n,t=>{this._trendDirection=t.direction,this._pastValueDisplay=t.pastValueDisplay});this._trendDirection=a?.direction??"unknown",this._pastValueDisplay=a?.pastValueDisplay??null}render(){if(!this._config||!this.hass)return q``;const t=this.hass.states[this._config.entity];if(!t)return q`<ha-card>
        <div class="warning">Entity not found: ${this._config.entity}</div>
      </ha-card>`;const e=(this.hass,this._config.entity,"%"),i=parseFloat(t.state),s=Number.isFinite(i),r=s?Ht(i):null,n={min:0,max:100},a=this._config.min??n.min,o=this._config.max??n.max,l=this._config.face_style??"classic",c=this._config.needle_color||Mt[l],h=this._config.name??t.attributes.friendly_name??this._config.entity;return q`
      <ha-card>
        <div class=${mt({"card-content":!0,[`style-${l}`]:!0})}>
          <div class="title">${h}</div>
          ${jt({min:a,max:o,value:r,pastValue:this._pastValueDisplay,needleColor:c})}
          <div class="readout">
            <span class="value">${s?r.toFixed(0):"--"}</span>
            <span class="unit">${e}</span>
          </div>
          <div class="trend trend-${this._trendDirection}">
            <span class="arrow">${Ft[this._trendDirection]}</span>
            <span class="label">${qt[this._trendDirection]}</span>
          </div>
        </div>
      </ha-card>
    `}};var Wt;Bt.styles=[Ct,a`
      :host {
        display: block;
      }
      .card-content {
    ...
    `],t([gt({attribute:!1})],Bt.prototype,"hass",void 0),t([pt()],Bt.prototype,"_config",void 0),t([pt()],Bt.prototype,"_trendDirection",void 0),t([pt()],Bt.prototype,"_pastValueDisplay",void 0),Bt=t([ut(Pt)],Bt),Wt={type:`custom:${Pt}`,name:"Analog Humidity (Radial)",preview:!1,description:"A classic analog-style radial hygrometer dial with humidity trend tracking."},window.customCards=window.customCards||[],window.customCards.push(Wt);export{Bt as AnalogHumidityRadialCard};
