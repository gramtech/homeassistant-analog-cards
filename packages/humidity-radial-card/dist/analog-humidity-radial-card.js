...
`;const Dt="analog-humidity-radial-card",Ot="analog-humidity-radial-card-editor",Tt="classic",jt={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Ht={startAngle:-135,sweepAngle:270},Nt=[{key:"dry",label:"Dry",from:0,to:30,color:"#b5651d"},{key:"comfortable",label:"Comfortable",from:30,to:50,color:"#4a7a3c"},{key:"humid",label:"Humid",from:50,to:70,color:"#c9a227"},{key:"very-humid",label:"Very Humid",from:70,to:100,color:"#2471a3"}],Rt={major:10,minor:2},Lt=100;function It(t){const{min:e,max:i,value:s,pastValue:r,needleColor:n}=t;return B`
  ...
  `}</defs>
      ...
      ${function(t,e){return Nt.map(i=>{if(i.to<=t||i.from>=e)return B``;const s=kt(i.from,t,e,Ht),r=kt(i.to,t,e,Ht);if(r<=s)return B``;const n=yt(Lt,Lt,86,s,r);return B`<path
      d=${n}
      stroke=${i.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i)}
      ${function(t,e){const{major:i,minor:s}=Rt;return vt(t,e,i,s,Ht).map(t=>{const e=t.major?8:4,i=_t(Lt,Lt,72-e,t.angle),s=_t(Lt,Lt,72,t.angle),r=B`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const n=_t(Lt,Lt,50,t.angle),a=B`<text x=${n.x} y=${n.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${r}${a}`})}(e,i)}
      ${function(t,e){return Nt.map(i=>{const s=Math.max(i.from,t),r=Math.min(i.to,e);if(r<=s)return B``;const n=kt(s,t,e,Ht),a=kt(r,t,e,Ht),o=`zone-label-path-${i.key}`,l=yt(Lt,Lt,77,n,a);return B`
    ...
    `})}(e,i)}
      ${function(t,e,i){if(null===t)return B``;const s=kt(t,e,i,Ht),r=_t(Lt,Lt,88,s),n=_t(Lt,Lt,95,s);return B`<line
    x1=${r.x} y1=${r.y} x2=${n.x} y2=${n.y}
    class="trend-marker"
  />`}(r,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=kt(t,e,i,Ht);return B`<g class="needle" transform="rotate(${r} ${Lt} ${Lt})">
    <polygon
  ...
  `}function Vt(t,e,i){return t}const Ft=[(qt="sensor",{name:"entity",selector:{entity:{domain:qt}}}),St("name"),Ct("trend_hours",{min:1,max:12,step:1}),Ct("trend_threshold",{min:0,step:.5}),Pt("trend_text_size",wt),Pt("face_style",At),St("needle_color")];var qt;const Bt=(Wt={entity:"Humidity sensor entity",name:"Name (optional)",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",trend_text_size:"Trend text size",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>Wt[t.name]??t.name);var Wt;let Kt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?q`
      <ha-form
        .hass=${this.hass}
      ...
      ></ha-form>
    `:q``}_effectiveData(t){const e=t.face_style??Tt;return{...t,trend_hours:t.trend_hours??3,trend_threshold:t.trend_threshold??3,trend_text_size:t.trend_text_size??xt,face_style:e,needle_color:t.needle_color||jt[e]}}_valueChanged(t){!function(t,e,i={},s={}){const r=new CustomEvent(e,{bubbles:s.bubbles??!0,cancelable:s.cancelable??!1,composed:s.composed??!0,detail:i});t.dispatchEvent(r)}(this,"config-changed",{config:t.detail.value})}};t([gt({attribute:!1})],Kt.prototype,"hass",void 0),t([pt()],Kt.prototype,"_config",void 0),Kt=t([dt(Ot)],Kt);const Gt={rising:"↑",falling:"↓",steady:"→",unknown:""},Jt={rising:"Rising",falling:"Falling",steady:"Steady",unknown:""};let Zt=class extends ct{constructor(){super(...arguments),this._trendDirection="unknown",this._pastValueDisplay=null,this._tracker=new Mt("analog-humidity-radial-card-trend-",Vt,3e5),this._lastEntityUpdate=null}setConfig(t){if(!t.entity)throw new Error("You must specify a humidity sensor entity.");this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement(Ot)}static getStubConfig(t){const e=Object.values(t?.states??{}).find(t=>t.entity_id.startsWith("sensor.")&&"humidity"===t.attributes.device_class);return{type:"custom:analog-humidity-radial-card",entity:e?.entity_id??""}}willUpdate(t){t.has("hass")&&this._maybeUpdateTrend()}_maybeUpdateTrend(){if(!this.hass||!this._config?.entity)return;const t=this.hass.states[this._config.entity];if(!t)return;const e=parseFloat(t.state);if(!Number.isFinite(e))return;const i=(this.hass,this._config.entity,"%"),s=Vt(e),r=this._config.trend_hours??3,n=this._config.trend_threshold??3;if(this._lastEntityUpdate===t.last_updated)return;this._lastEntityUpdate=t.last_updated;const a=this._tracker.update(this.hass,this._config.entity,s,i,i,r,n,t=>{this._trendDirection=t.direction,this._pastValueDisplay=t.pastValueDisplay});this._trendDirection=a?.direction??"unknown",this._pastValueDisplay=a?.pastValueDisplay??null}render(){if(!this._config||!this.hass)return q``;const t=this.hass.states[this._config.entity];if(!t)return q`<ha-card>
        <div class="warning">Entity not found: ${this._config.entity}</div>
      </ha-card>`;const e=(this.hass,this._config.entity,"%"),i=parseFloat(t.state),s=Number.isFinite(i),r=s?Vt(i):null,n={min:0,max:100},a=this._config.min??n.min,o=this._config.max??n.max,l=this._config.face_style??Tt,c=this._config.needle_color||jt[l],h=this._config.name??t.attributes.friendly_name??this._config.entity,d=Et[this._config.trend_text_size??xt];return q`
      <ha-card>
        <div class=${$t({"card-content":!0,[`style-${l}`]:!0})}>
          <div class="title">${h}</div>
          ${It({min:a,max:o,value:r,pastValue:this._pastValueDisplay,needleColor:c})}
          <div class="readout">
    ...
    `],t([gt({attribute:!1})],Zt.prototype,"hass",void 0),t([pt()],Zt.prototype,"_config",void 0),t([pt()],Zt.prototype,"_trendDirection",void 0),t([pt()],Zt.prototype,"_pastValueDisplay",void 0),Zt=t([dt(Dt)],Zt),Yt={type:Dt,name:"Analog Humidity (Radial)",preview:!1,description:"A classic analog-style radial hygrometer dial with humidity trend tracking."},window.customCards=window.customCards||[],window.customCards.push(Yt);export{Zt as AnalogHumidityRadialCard};
