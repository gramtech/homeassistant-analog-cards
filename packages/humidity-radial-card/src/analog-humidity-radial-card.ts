...
import { HomeAssistant, AnalogHumidityRadialCardConfig, TrendDirection } from './types';
import { convert, detectUnit, getDefaultRange, getDefaultThreshold } from './unit-utils';
import './analog-humidity-radial-card-editor';
...
export class AnalogHumidityRadialCard extends LitElement {
  ...
  setConfig(config: AnalogHumidityRadialCardConfig): void {
    if (!config.entity) {
      throw new Error('You must specify a humidity sensor entity.');
    }
    this._config = config;
  }
  ...
  static getStubConfig(hass: HomeAssistant): Partial<AnalogHumidityRadialCardConfig> {
    const match = Object.values(hass?.states ?? {}).find(
      (entity) =>
        entity.entity_id.startsWith('sensor.') && entity.attributes.device_class === 'humidity'
    );
    return { type: 'custom:analog-humidity-radial-card', entity: match?.entity_id ?? '' };
  }
  ...
  private _maybeUpdateTrend(): void {
    ...
    const currentDisplay = convert(rawValue, nativeUnit, nativeUnit);
    const hoursAgo = this._config.trend_hours ?? DEFAULT_TREND_HOURS;
    const threshold = this._config.trend_threshold ?? getDefaultThreshold(nativeUnit);
  ...
  protected render() {
    ...
    const currentDisplay = hasValue ? convert(rawValue, nativeUnit, nativeUnit) : null;

    const defaultRange = getDefaultRange(nativeUnit);
    ...
    // needle color is driven by face style rather than the HA theme.
    ...
    return html`
      <ha-card>
        <div class=${classMap({ 'card-content': true, [`style-${faceStyle}`]: true })}>
          ...
          <div class="readout">
            <span class="value">${hasValue ? currentDisplay!.toFixed(0) : '--'}</span>
            <span class="unit">${nativeUnit}</span>
          </div>
...
declare global {
  interface HTMLElementTagNameMap {
    [CARD_TAG]: AnalogHumidityRadialCard;
  }
}
