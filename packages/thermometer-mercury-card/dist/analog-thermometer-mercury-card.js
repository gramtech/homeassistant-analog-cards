...
`,Mt="analog-thermometer-mercury-card",Ut="analog-thermometer-mercury-card-editor",zt={classic:"#c0392b",silver:"#c9cdd1",dark:"#e05c4d",light:"#c0392b","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Ot={"°C":{major:10,minor:2},"°F":{major:20,minor:5}},Dt=60,Tt=20,jt=168,Nt=10,Ht=jt-Math.sqrt(300),Rt="mercury-liquid-clip";function Ft(t,e){const i=Dt-t,s=Dt+t,r=Math.sqrt(e*e-t*t),n=jt-r;return[`M ${i} 20`,`L ${i} ${n}`,`A ${e} ${e} 0 1 0 ${s} ${n}`,`L ${s} 20`,`A ${t} ${t} 0 1 0 ${i} 20`,"Z"].join(" ")}function qt(t){const{min:e,max:i,unit:s,value:r,pastValue:n,liquidColor:a}=t;return B`
  ...
  `}</defs>
      ...
      ${function(t,e,i){const{major:s,minor:r}=Ot[i];return yt(t,e,s,r,Ht,Tt).map(t=>{const e=t.major?8:4,i=B`<line
    ...
    />`;if(!t.major)return i;const s=B`<text x=${84} y=${t.position} class="tick-label" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${i}${s}`})}(e,i,s)}
      ${function(t,e,i){if(null===t)return B``;const s=_t(t,e,i,Ht,Tt);return B`<line
    x1=${42} y1=${s} x2=${48} y2=${s}
    class="trend-marker"
  />`}(n,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=_t(t,e,i,Ht,Tt);return B`
    <defs>
      <clipPath id=${Rt}>
        <rect x="0" y=${r} width="140" height=${192-r} />
      </clipPath>
    </defs>
    <path d=${Ft(6,16)} class="liquid" fill=${s} clip-path=${`url(#${Rt})`} />
  `}(r,e,i,a)}
      <path d=${Ft(Nt,20)} class="glass" />
      <line
        x1=${53} y1=${26}
        x2=${53} y2=${Ht-6}
  ...
  `}function Lt(t){return 9*t/5+32}function It(t,e,i){return e===i?t:(s=function(t,e){return"°F"===e?function(t){return 5*(t-32)/9}(t):t}(t,e),"°F"===i?Lt(s):s);var s}function Vt(t,e){const i=t.states[e]?.attributes?.unit_of_measurement;return i&&i.toLowerCase().includes("f")?"°F":"°C"}const Bt=[(Wt="sensor",{name:"entity",selector:{entity:{domain:Wt}}}),wt("name"),Et("unit",[{value:"auto",label:"Auto-detect"},{value:"°C",label:"°C"},{value:"°F",label:"°F"}]),xt("trend_hours",{min:1,max:12,step:1}),xt("trend_threshold",{min:0,step:.1}),Et("trend_text_size",vt),Et("face_style",kt),wt("liquid_color")];var Wt;const Kt=(Gt={entity:"Temperature sensor entity",name:"Name (optional)",unit:"Display unit",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",trend_text_size:"Trend text size",face_style:"Face style",liquid_color:"Liquid color (optional, overrides face style default)"},t=>Gt[t.name]??t.name);var Gt;let Jt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?V`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Bt}
    ...
    `],t([gt({attribute:!1})],Qt.prototype,"hass",void 0),t([pt()],Qt.prototype,"_config",void 0),t([pt()],Qt.prototype,"_trendDirection",void 0),t([pt()],Qt.prototype,"_pastValueDisplay",void 0),Qt=t([ut(Mt)],Qt),Xt={type:Mt,name:"Analog Thermometer (Mercury)",preview:!1,description:"A classic mercury-tube style thermometer with temperature trend tracking."},window.customCards=window.customCards||[],window.customCards.push(Xt);export{Qt as AnalogThermometerMercuryCard};
