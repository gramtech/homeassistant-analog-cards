...
`,Ct="analog-thermometer-mercury-card",Pt="analog-thermometer-mercury-card-editor",Ut={classic:"#c0392b",silver:"#c9cdd1",dark:"#e05c4d",light:"#c0392b","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Mt={"°C":{major:10,minor:2},"°F":{major:20,minor:5}},Ot=60,Dt=185;function zt(t){const{min:e,max:i,unit:s,value:r,pastValue:n,liquidColor:o}=t;return B`
  ...
  `}</defs>
      ...
      ${function(t,e,i){const{major:s,minor:r}=Mt[i];return yt(t,e,s,r,Dt,16).map(t=>{const e=t.major?8:4,i=B`<line
      x1=${74} y1=${t.position} x2=${74+e} y2=${t.position}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return i;const s=B`<text x=${86} y=${t.position} class="tick-label" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${i}${s}`})}(e,i,s)}
      ${function(t,e,i){if(null===t)return B``;const s=_t(t,e,i,Dt,16);return B`<line
    x1=${40} y1=${s} x2=${46} y2=${s}
    class="trend-marker"
  />`}(n,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=_t(t,e,i,Dt,16),n=Math.max(0,193-r);return B`
    <rect
      x=${53}
      y=${r}
      width=${14}
      ...
      fill=${s}
    />
    <circle cx=${Ot} cy=${205} r=${20} class="bulb" fill=${s} />
  `}(r,e,i,o)}
      <rect
        x=${48}
        y=${16}
        ...
        class="glass"
      />
      <circle cx=${Ot} cy=${205} r=${23} class="glass-bulb-ring" />
      <ellipse cx=${55} cy=${199} rx="4" ry="7" class="bulb-highlight" />
    </svg>
  `}function Tt(t){return 9*t/5+32}function jt(t,e,i){return e===i?t:(s=function(t,e){return"°F"===e?function(t){return 5*(t-32)/9}(t):t}(t,e),"°F"===i?Tt(s):s);var s}function Nt(t,e){const i=t.states[e]?.attributes?.unit_of_measurement;return i&&i.toLowerCase().includes("f")?"°F":"°C"}const Ht=[(Rt="sensor",{name:"entity",selector:{entity:{domain:Rt}}}),vt("name"),wt("unit",[{value:"auto",label:"Auto-detect"},{value:"°C",label:"°C"},{value:"°F",label:"°F"}]),At("trend_hours",{min:1,max:12,step:1}),At("trend_threshold",{min:0,step:.1}),wt("face_style",kt),vt("liquid_color")];var Rt;const Ft=(qt={entity:"Temperature sensor entity",name:"Name (optional)",unit:"Display unit",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",face_style:"Face style",liquid_color:"Liquid color (optional, overrides face style default)"},t=>qt[t.name]??t.name);var qt;let Lt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?V`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Ht}
    ...
    `],t([gt({attribute:!1})],Bt.prototype,"hass",void 0),t([pt()],Bt.prototype,"_config",void 0),t([pt()],Bt.prototype,"_trendDirection",void 0),t([pt()],Bt.prototype,"_pastValueDisplay",void 0),Bt=t([ut(Ct)],Bt),Wt={type:Ct,name:"Analog Thermometer (Mercury)",description:"A classic mercury-tube style thermometer with temperature trend tracking."},window.customCards=window.customCards||[],window.customCards.push(Wt);export{Bt as AnalogThermometerMercuryCard};
