...
import { DEFAULT_FACE_STYLE, DEFAULT_TREND_HOURS, EDITOR_TAG, LIQUID_COLOR_BY_STYLE } from './const';
import { computeLabel, SCHEMA } from './editor-schema';
import { fireEvent, HomeAssistant, AnalogThermometerMercuryCardConfig } from './types';
...
export class AnalogThermometerMercuryCardEditor extends LitElement {
  ...
  private _effectiveData(
    hass: HomeAssistant,
    config: AnalogThermometerMercuryCardConfig
  ): AnalogThermometerMercuryCardConfig {
    ...
    return {
      ...
      liquid_color: config.liquid_color || LIQUID_COLOR_BY_STYLE[faceStyle],
    };
  }

  private _valueChanged(ev: CustomEvent<{ value: AnalogThermometerMercuryCardConfig }>): void {
