...
`;const Dt="analog-thermometer-radial-card",Ot="analog-thermometer-radial-card-editor",Tt="classic",jt={classic:"#333333",silver:"#222222",dark:"#e8e8e8",light:"#333333","led-backlit":"#ff4d4d",fluorescent:"#baff29",futuristic:"#ff2fd0"},Nt={startAngle:-135,sweepAngle:270},Ht=[{key:"freezing",label:"Freezing",from:-20,to:0,color:"#2471a3"},{key:"cold",label:"Cold",from:0,to:15,color:"#5dade2"},{key:"comfortable",label:"Comfortable",from:15,to:25,color:"#4a7a3c"},{key:"warm",label:"Warm",from:25,to:32,color:"#c9a227"},{key:"hot",label:"Hot",from:32,to:45,color:"#b5651d"}],Rt={"°C":{major:10,minor:2},"°F":{major:20,minor:5}};function Ft(t){return 9*t/5+32}function Lt(t,e){return"°F"===e?Ft(t):t}function It(t,e,i){return e===i?t:Lt(function(t,e){return"°F"===e?function(t){return 5*(t-32)/9}(t):t}(t,e),i)}function Vt(t,e){return t&&"auto"!==t?t:e}function qt(t,e){const i=t.states[e]?.attributes?.unit_of_measurement;return i&&i.toLowerCase().includes("f")?"°F":"°C"}function Bt(t){return"°F"===t?1.8:1}const Wt=100;function Kt(t){const{min:e,max:i,unit:s,value:r,pastValue:n,needleColor:a}=t;return B`
  ...
  `}</defs>
      <circle cx=${Wt} cy=${Wt} r="97" class="bezel" />
      <circle cx=${Wt} cy=${Wt} r="90" class="face" />
      ${function(t,e,i){return Ht.map(s=>{const r=Lt(s.from,i),n=Lt(s.to,i);if(n<=t||r>=e)return B``;const a=kt(r,t,e,Nt),o=kt(n,t,e,Nt);if(o<=a)return B``;const l=yt(Wt,Wt,86,a,o);return B`<path
      d=${l}
      stroke=${s.color}
      stroke-width=${9}
      fill="none"
    />`})}(e,i,s)}
      ${function(t,e,i){const{major:s,minor:r}=Rt[i];return vt(t,e,s,r,Nt).map(t=>{const e=t.major?8:4,i=_t(Wt,Wt,72-e,t.angle),s=_t(Wt,Wt,72,t.angle),r=B`<line
      x1=${i.x} y1=${i.y} x2=${s.x} y2=${s.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return r;const n=_t(Wt,Wt,50,t.angle),a=B`<text x=${n.x} y=${n.y} class="tick-label" text-anchor="middle" dominant-baseline="middle">${Math.round(10*t.value)/10}</text>`;return B`${r}${a}`})}(e,i,s)}
      ${function(t,e,i){return Ht.map(s=>{const r=Math.max(Lt(s.from,i),t),n=Math.min(Lt(s.to,i),e);if(n<=r)return B``;const a=kt(r,t,e,Nt),o=kt(n,t,e,Nt),l=`zone-label-path-${s.key}`,c=yt(Wt,Wt,77,a,o);return B`
    ...
    `})}(e,i,s)}
      ${function(t,e,i){if(null===t)return B``;const s=kt(t,e,i,Nt),r=_t(Wt,Wt,88,s),n=_t(Wt,Wt,95,s);return B`<line
    x1=${r.x} y1=${r.y} x2=${n.x} y2=${n.y}
    class="trend-marker"
  />`}(n,e,i)}
      ${function(t,e,i,s){if(null===t)return B``;const r=kt(t,e,i,Nt);return B`<g class="needle" transform="rotate(${r} ${Wt} ${Wt})">
    <polygon
      points="${Wt},${38} ${97},${114} ${103},${114}"
      fill=${s}
    />
  </g>`}(r,e,i,a)}
      <circle cx=${Wt} cy=${Wt} r="6" class="hub" />
    </svg>
  `}const Gt=[(Jt="sensor",{name:"entity",selector:{entity:{domain:Jt}}}),St("name"),zt("unit",[{value:"auto",label:"Auto-detect"},{value:"°C",label:"°C"},{value:"°F",label:"°F"}]),Ct("trend_hours",{min:1,max:12,step:1}),Ct("trend_threshold",{min:0,step:.1}),zt("trend_text_size",wt),zt("face_style",At),St("needle_color")];var Jt;const Zt=(Yt={entity:"Temperature sensor entity",name:"Name (optional)",unit:"Display unit",trend_hours:"Trend lookback (hours)",trend_threshold:"Trend threshold",trend_text_size:"Trend text size",face_style:"Face style",needle_color:"Needle color (optional, overrides face style default)"},t=>Yt[t.name]??t.name);var Yt;let Qt=class extends ct{setConfig(t){this._config=t}render(){return this.hass&&this._config?q`
      <ha-form
        .hass=${this.hass}
      ...
      ></ha-form>
    `:q``}_effectiveData(t,e){const i=e.entity?qt(t,e.entity):"°C",s=Vt(e.unit,i),r=e.face_style??Tt;return{...e,unit:e.unit??"auto",trend_hours:e.trend_hours??3,trend_threshold:e.trend_threshold??Bt(s),trend_text_size:e.trend_text_size??xt,face_style:r,needle_color:e.needle_color||jt[r]}}_valueChanged(t){!function(t,e,i={},s={}){const r=new CustomEvent(e,{bubbles:s.bubbles??!0,cancelable:s.cancelable??!1,composed:s.composed??!0,detail:i});t.dispatchEvent(r)}(this,"config-changed",{config:t.detail.value})}};t([gt({attribute:!1})],Qt.prototype,"hass",void 0),t([pt()],Qt.prototype,"_config",void 0),Qt=t([ut(Ot)],Qt);const Xt={rising:"↑",falling:"↓",steady:"→",unknown:""},te={rising:"Rising",falling:"Falling",steady:"Steady",unknown:""};let ee=class extends ct{constructor(){super(...arguments),this._trendDirection="unknown",this._pastValueDisplay=null,this._tracker=new Mt("analog-thermometer-radial-card-trend-",It,3e5),this._lastEntityUpdate=null}setConfig(t){if(!t.entity)throw new Error("You must specify a temperature sensor entity.");this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement(Ot)}static getStubConfig(t){const e=Object.values(t?.states??{}).find(t=>t.entity_id.startsWith("sensor.")&&"temperature"===t.attributes.device_class);return{type:"custom:analog-thermometer-radial-card",entity:e?.entity_id??""}}willUpdate(t){t.has("hass")&&this._maybeUpdateTrend()}_maybeUpdateTrend(){if(!this.hass||!this._config?.entity)return;const t=this.hass.states[this._config.entity];if(!t)return;const e=parseFloat(t.state);if(!Number.isFinite(e))return;const i=qt(this.hass,this._config.entity),s=Vt(this._config.unit,i),r=It(e,i,s),n=this._config.trend_hours??3,a=this._config.trend_threshold??Bt(s);if(this._lastEntityUpdate===t.last_updated)return;this._lastEntityUpdate=t.last_updated;const o=this._tracker.update(this.hass,this._config.entity,r,i,s,n,a,t=>{this._trendDirection=t.direction,this._pastValueDisplay=t.pastValueDisplay});this._trendDirection=o?.direction??"unknown",this._pastValueDisplay=o?.pastValueDisplay??null}render(){if(!this._config||!this.hass)return q``;const t=this.hass.states[this._config.entity];if(!t)return q`<ha-card>
        <div class="warning">Entity not found: ${this._config.entity}</div>
      </ha-card>`;const e=qt(this.hass,this._config.entity),i=Vt(this._config.unit,e),s=parseFloat(t.state),r=Number.isFinite(s),n=r?It(s,e,i):null,a="°F"===i?{min:Ft(-20),max:Ft(45)}:{min:-20,max:45};const o=this._config.min??a.min,l=this._config.max??a.max,c=this._config.face_style??Tt,h=this._config.needle_color||jt[c],u=this._config.name??t.attributes.friendly_name??this._config.entity,d=Et[this._config.trend_text_size??xt];return q`
      <ha-card>
        <div class=${$t({"card-content":!0,[`style-${c}`]:!0})}>
          <div class="title">${u}</div>
          ${Kt({min:o,max:l,unit:i,value:n,pastValue:this._pastValueDisplay,needleColor:h})}
          <div class="readout">
          ...
          <div class="trend trend-${this._trendDirection}" style="font-size: ${d}rem">
            <span class="arrow">${Xt[this._trendDirection]}</span>
            <span class="label">${te[this._trendDirection]}</span>
          </div>
        </div>
      </ha-card>
    `}};var ie;ee.styles=[Ut,a`
      :host {
        display: block;
      }
      .card-content {
    ...
    `],t([gt({attribute:!1})],ee.prototype,"hass",void 0),t([pt()],ee.prototype,"_config",void 0),t([pt()],ee.prototype,"_trendDirection",void 0),t([pt()],ee.prototype,"_pastValueDisplay",void 0),ee=t([ut(Dt)],ee),ie={type:Dt,name:"Analog Thermometer (Radial)",preview:!1,description:"A classic analog-style radial thermometer dial with temperature trend tracking."},window.customCards=window.customCards||[],window.customCards.push(ie);export{ee as AnalogThermometerRadialCard};
