...
export class AnalogThermometerRadialCard extends LitElement {
  ...
  static getStubConfig(hass: HomeAssistant): Partial<AnalogThermometerRadialCardConfig> {
    const match = Object.values(hass?.states ?? {}).find(
      (entity) =>
        entity.entity_id.startsWith('sensor.') && entity.attributes.device_class === 'temperature'
    );
    return { type: 'custom:analog-thermometer-radial-card', entity: match?.entity_id ?? '' };
  }
...
declare global {
  interface HTMLElementTagNameMap {
    [CARD_TAG]: AnalogThermometerRadialCard;
  }
}
