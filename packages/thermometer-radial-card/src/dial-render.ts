...
import { DIAL_SCALE, TICK_STEPS, ZONES_CELSIUS } from './const';
import { TemperatureUnit } from './types';
import { convertFromCelsius } from './unit-utils';
...
function renderZoneArcs(min: number, max: number, unit: TemperatureUnit): SVGTemplateResult[] {
  return ZONES_CELSIUS.map((zone) => {
    const from = convertFromCelsius(zone.from, unit);
    const to = convertFromCelsius(zone.to, unit);
...
function renderZoneLabels(min: number, max: number, unit: TemperatureUnit): SVGTemplateResult[] {
  return ZONES_CELSIUS.map((zone) => {
    const from = Math.max(convertFromCelsius(zone.from, unit), min);
    const to = Math.min(convertFromCelsius(zone.to, unit), max);
